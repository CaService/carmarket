...
ini_set('error_log', __DIR__ . '/reset_password_errors.log');

error_log("=== Inizio richiesta reset password ===");
...
try {
    ...
    <body>
        <div class='container'>
            ...
            <div class='footer'>
                <p>Questa richiesta è stata inviata tramite il form di reset password di Carmarket Ayvens.</p>
            </div>
        </div>
    </body>
    </html>";
...
Questa richiesta è stata inviata tramite il form di reset password di Carmarket Ayvens.";

    error_log("Tentativo di invio email...");
    $mail->send();
    error_log("Email inviata con successo");
    
    echo json_encode(['status' => 'success', 'message' => 'Richiesta di reset password inviata con successo.']);
    ...
    echo json_encode([
        'status' => 'error',
        'message' => "Impossibile inviare la richiesta di reset password: " . $errorMessage
    ]);
}

error_log("=== Fine richiesta reset password ===");
?> 